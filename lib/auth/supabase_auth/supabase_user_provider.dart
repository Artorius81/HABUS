...
class HabusSupabaseUser extends BaseAuthUser {
  HabusSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> habusSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = HabusSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
